...
class FormResult extends Component {
  ...
  handleBindData() {
    this.props.SpiralFormController.GetResultById(this.props.dataInput.id).then(
      () => {
        this.setState({ loading: false });
        const result = this.props.formResult;
        ...
        this.setState({
          ...
          checkList: lst,
          //   employees:
          //     result.employees !== null ? JSON.parse(result.employees) : null,
        });
      }
    );
  }
      ...
      switch (answerType) {
        ...
        case 5: // radio
          if (answer) {
            ...
            let goTo = question.questionId;
            let maxGoTo = question.questionId;
            ...
            if (question.questionId < maxGoTo) {
              ...
              for (let index = question.questionId; index < maxIndex; index++) {
                if (checkList[index].questionId < goTo)
                  checkList[index].isShow = false;
                else checkList[index].isShow = true;
            ...
            ///----------- FORM 17
            // if (question.questionId === 1 && this.state.formData.id === 17) {
            //   question.anwserItem.forEach((element) => {
            //     if (element.anwserValue && element.anwserName === "No") {
            //       for (let index = 2; index < checkList.length; index++) {
            //         checkList[index].isShow = false;
            //       }
            //     } else if (
            //       element.anwserValue &&
            //       element.anwserName === "Yes"
            //     ) {
            //       for (let index = 2; index < checkList.length; index++) {
            //         checkList[index].isShow = true;
            //       }
            ...
            // }
            // ///----------- FORM 30
            // if (question.questionId === 1 && this.state.formData.id === 30) {
            //   question.anwserItem.forEach((element) => {
            //     if (element.anwserValue && element.anwserName === "A") {
            //       for (let index = 1; index < checkList.length; index++) {
            //         if (checkList[index].questionId > 20)
            //           checkList[index].isShow = false;
            //       }
            //     } else if (element.anwserValue && element.anwserName === "B") {
            //       for (let index = 1; index < checkList.length; index++) {
            //         if (checkList[index].questionId > 20)
            //           checkList[index].isShow = true;
            //       }
        ...
        divImages.push(
          <div key={item} style={{ display: "inline-flex" }}>
            <img
              ...
              height={70}
              src={item}
              onClick={(e) => this.handleImage(images, e)}
            ></img>
            <Button
              onClick={() =>
                this.handleDeleteImage(question.questionId, item, "images")
  ...
  handleOnSubmit() {
    ...
    if (formData.length > 0) {
      ...
      formData.forEach((question) => {
        ...
        if (question.isEnd) questionEnd = question.questionId;
            ...
            if (
              ...
              switch (answers.anwserType) {
                ...
                case 10:
                  ...
                  break;
                default:
                  if (question.questionName !== "Ghi chú") {
                    if (answers.anwserValue === "") {
                      isCheck = false;
                      this.showError(
                        "Chưa trả lời câu hỏi: " + question.questionName
    ...
    if (isCheck) {
      ...
      let data = {
        id: dataResult.id,
        formData: dataResult.formData,
      };
      // formData.forEach(element => {
      //     this.showSuccess(element.anwserItem[0].anwserValue);
      // });
      this.props.SpiralFormController.UpdateResult(data).then(() => {
  ...
  handleImage = (image, event) => {
    ...
    image.forEach((element) => {
      results.push({
        url: element.imageURL || element,
        title: "Image " + count + "/" + total,
      });
      count++;
    });
    this.setState({ itemImage: results, isOpen: true, indexActive: 0 });
  };
  renderBody() {
    let result = [];
        ...
        for (let index = 0; index < formData.length; index++) {
          ...
          if (isCheck.isShow) {
            const isEnd = this.state.checkList.find(
              (item) => item.isShow && (item.isEnd || item.isBreak)
            );
            if (isEnd !== undefined) {
              if (
  ...
  renderFooter() {
    return (
      <div className={"p-col-7"} style={{ position: "fixed", bottom: -8 }}>
  ...
  render() {
    ...
    let body = this.renderBody(),
      ...
      userInfo = null; // this.renderUserInfo();
    if (formData.id === 0) {
      return (
        <div>
    ...
    return (
      <>
        ...
        <div key="container" className="p-d-flex p-jc-center">
          ...
          {this.props.permission?.edit && footer}
        </div>
        {this.state.isOpen ? (
          <Lightbox
