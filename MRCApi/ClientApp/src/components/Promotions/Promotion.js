...
import PhotoGalary from '../Controls/Gallery/PhotoGallery';
...
class Promotion extends PureComponent {
    ...
    Search = async (data) => {
        await this.setState({ expandedRows: null, isLoading: true })
        await this.props.PromotionController.FilterPromotion(data).then(() => {
            const result = this.props.filterPromotion
    ...
    Export = async (data) => {
        await this.props.PromotionController.ExportPromotion(data)
        const result = this.props.exportPromotion
    ...
    totalTemplate = (rowData) => {
        return (<div style={{ textAlign: "center" }}>
            ...
            <label >({rowData.actual}/{rowData.total})</label>
        </div>
        );
    }
    render() {
        ...
        result =
            <DataTable
                ...
                <Column filter field="areaName" style={{ width: 80 }} header={this.props.language["area"] || "l_area"} />
                <Column filter field="customerName" style={{ width: 120 }} header={this.props.language["customer_name"] || "l_customer_name"} />
                <Column filter field="accountName" style={{ width: 100 }} header={this.props.language["account"] || "l_account"} />
                <Column filter body={this.shopNameTemplate} header={this.props.language["shop_name"] || "l_shop_name"} style={{ width: "220px" }} />
                <Column filter field="shopAddress" header={this.props.language["address"] || "address"} />
                ...
                <Column body={this.totalTemplate} header={this.props.language["total"] || "l_total"} style={{ width: 100 }} />
            </DataTable>
...
function mapStateToProps(state) {
    return {
        ...
        filterPromotion: state.promotion.filterPromotion,
        exportPromotion: state.promotion.exportPromotion,
    }
}
