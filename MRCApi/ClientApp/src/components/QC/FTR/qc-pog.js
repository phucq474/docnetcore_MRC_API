...
class QCPOG extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            activeIndex: 0,
    ...
    onChangeByQC(id, value, type) {
        ...
        switch (type) {
            ...
            case 'qcValue':
                ...
                break;
            case 'qcpog':
                qcDetail[index].qcpog = value === true ? 1 : (qcDetail[index].qcpog === 1 && value !== true) ? 0 : null;
                if (value)
                    qcDetail[index].qcNoImage = null;
                break;
            default:
    ...
    qcPOGTemplate = (rowData) => {

        return (
            <Checkbox
                checked={rowData.qcpog === 1 ? true : false}
                onChange={(e) => this.onChangeByQC(rowData.id, e.target.checked, 'qcpog')}
                style={{ opacity: 1 }} />
        )
    }
                    ...
                    <div className="p-col-12 p-md-7 p-sm-12">
                        <DataTable className="p-datatable-striped"
                            ...
                            {getLogin().accountName === "Fonterra" &&
                                <Column filter={true} body={this.qcPOGTemplate} field='qcpog' style={{ width: 70, textAlign: 'center' }} header="QC POG" />}
                        </DataTable>
                    </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(QCPOG);