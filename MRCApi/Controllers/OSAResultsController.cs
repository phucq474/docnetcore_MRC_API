...
{
    public class OSAResultsController : SpiralBaseController
    {
        private readonly IOSAResultsService _service;
        private readonly IWebHostEnvironment _webHostEnvironment;

        public OSAResultsController(OSAResultsContext context, IWebHostEnvironment webHost)
        {
            _service = new OSAResultsService(context);
        ...
        public async Task<ResultInfo> Export([FromHeader] string JsonData, [FromHeader] int? accId)
        {
            try
            {
                ...
                FileInfo file = null;
                string fileName = $"OSAResult_{DateTime.Now.ToString("ddMMyyyy_HHmmss")}.xlsx";
                    ...
                    using (var package = new ExcelPackage(file))
                    {
                        ...
                        if (sheet != null)
                        {
                            if (dt.Tables[0].Rows.Count > 0)
                            {
                                ...
                                sheet.Cells[1, 1].Value = "OSA REPORT";
